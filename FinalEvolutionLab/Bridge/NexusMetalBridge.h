...
/// Binds a CAMetalLayer (e.g. from MTKView) and configures the mesh-backed Metal renderer.
bool nexus_metal_renderer_initialize(NexusMetalRendererHandle _Nullable handle,
                                     CAMetalLayer *_Nullable layer,
                                     uint32_t width,
                                     uint32_t height);

/// Draws one frame of the active arena scene. Safe to call from MTKViewDelegate.
