...
/// Binds a CAMetalLayer (e.g. from MTKView) and loads the active NEXUS venue scene.
bool nexus_metal_renderer_initialize(NexusMetalRendererHandle _Nullable handle,
                                     CAMetalLayer *_Nullable layer,
                                     uint32_t width,
                                     uint32_t height);

/// Draws one venue frame from the bundled NEXUS manifest. Safe to call from MTKViewDelegate.
