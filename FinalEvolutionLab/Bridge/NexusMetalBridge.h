...
/// Binds a CAMetalLayer (e.g. from MTKView) and configures the manifest-backed Metal renderer.
bool nexus_metal_renderer_initialize(NexusMetalRendererHandle _Nullable handle,
                                     CAMetalLayer *_Nullable layer,
                                     uint32_t width,
                                     uint32_t height);

/// Draws one frame from bundled NEXUS meshes; falls back to a clear color if no mesh is resolved.
