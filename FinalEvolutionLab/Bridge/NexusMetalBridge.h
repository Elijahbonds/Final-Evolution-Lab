...
/// Binds a CAMetalLayer (e.g. from MTKView) and configures the bundled NEXUS venue renderer.
bool nexus_metal_renderer_initialize(NexusMetalRendererHandle _Nullable handle,
                                     CAMetalLayer *_Nullable layer,
                                     uint32_t width,
                                     uint32_t height);

/// Draws one venue frame from the bundled manifest/mesh scene batch. Safe to call from MTKViewDelegate.
