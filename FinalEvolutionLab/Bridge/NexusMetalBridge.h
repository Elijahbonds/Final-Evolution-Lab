...
/// Binds a CAMetalLayer (e.g. from MTKView) and loads the mode's bundled venue mesh when present.
bool nexus_metal_renderer_initialize(NexusMetalRendererHandle _Nullable handle,
                                     CAMetalLayer *_Nullable layer,
                                     uint32_t width,
                                     uint32_t height);

/// Draws one Metal frame for the active venue mesh, falling back to a clear frame when no mesh is available.
