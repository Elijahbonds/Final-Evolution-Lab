...
final class NEXUSAgentService {
    ...
    private func launchMode(arguments: [String: Any]) -> NEXUSAgentToolResult {
        ...
        guard mode.id.isNexusRuntimeLaunchable else {
            return failure(.launchMode, "\(modeId) is not launchable through the NEXUS runtime")
        }
        ...
        return NEXUSAgentToolResult(
            ...
            payload: [
                "mode_id": mode.id.rawValue,
                "registry_mode_id": modeId,
                "runtime_mode_id": mode.id.nexusRuntimeModeId,
                "mode_name": mode.name,
                "readiness": readiness,
