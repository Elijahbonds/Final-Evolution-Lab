...
struct LabView: View {
    ...
    private static let welcomeLabShardsGrantedKey = "felDidGrantWelcomeLabShards"

    private func grantWelcomeLabShardsIfNeeded() {
        let d = UserDefaults.standard
        guard !d.bool(forKey: Self.welcomeLabShardsGrantedKey) else { return }
        d.set(true, forKey: Self.welcomeLabShardsGrantedKey)
        viewModel.profile.evolutionShards += 500
        SaveSystem.saveProfile(viewModel.profile)
        d.set("Welcome to the Lab — +500 Evolution Shards", forKey: PRQManager.pendingLabWelcomeToastKey)
    ...
    private var cloudCortexSection: some View {
        VStack(alignment: .leading, spacing: 0) {
            HStack(spacing: 8) {
                ...
                Button {
                    Task {
                        await prqManager.fetchAICoachInsight(metrics: PerformanceSnapshot.make(from: viewModel))
                    }
                } label: {
                    if prqManager.cloudCortexIsLoading {
                        ProgressView()
                            .scaleEffect(0.85)
                    } else {
                        Text("Refresh")
                ...
                .disabled(prqManager.cloudCortexIsLoading || !GeminiService.shared.isConfigured)
            }
            ...
            VStack(alignment: .leading, spacing: 8) {
                if let err = prqManager.cloudCortexError {
                    Text(err)
                        .font(.system(size: 11, weight: .medium, design: .monospaced))
                        .foregroundStyle(.orange.opacity(0.95))
                        .lineLimit(4)
                } else if let rx = prqManager.cloudCortexPrescription {
                    Text(rx)
                        .font(.system(size: 13, weight: .medium, design: .monospaced))
                        .foregroundStyle(.white.opacity(0.95))
                        .lineLimit(5)
                        .minimumScaleFactor(0.85)
                    if let t = prqManager.cloudCortexLastUpdated {
                        Text(t.formatted(date: .abbreviated, time: .shortened))
                            .font(.system(size: 9, weight: .medium, design: .monospaced))
                            .foregroundStyle(.tertiary)
                    }
                } else {
                    Text(GeminiService.shared.isConfigured
                        ? "Tap Refresh — Neuro-Mechanic line (HUD, non-blocking)."
                        : "Add GEMINI_API_KEY to enable Cloud Cortex.")
        ...
        .shadow(color: Theme.brandCyan.opacity(0.08), radius: 12, y: 4)
    }
    ...
    private func executeFreestyleScoring() {
        ...
        let result = freestyleDunk.calculateDunkScore(
            prq: prq,
            neuralBurst: burst,
    ...
    private var pressureManagementSection: some View {
        VStack(alignment: .leading, spacing: 12) {
            Text("SENSORY MAPPING — BUILD 6")
                .font(.system(.caption2, design: .monospaced, weight: .bold))
                .foregroundStyle(Theme.brandCyan)
                .tracking(2)
            Button {
                showPressureManagement = true
            } label: {
                HStack {
                    ...
                    VStack(alignment: .leading, spacing: 4) {
                        Text("Pressure Management")
                            ...
                            .foregroundStyle(.white)
                        Text("Breath, 90/90 rotation, SFMA GPS, fascial sling preview")
                            .font(.caption)
                            .foregroundStyle(.secondary)
                    }
                    Spacer()
                ...
                .background(
                    RoundedRectangle(cornerRadius: 16)
                        ...
                        .overlay(
                            RoundedRectangle(cornerRadius: 16)
                                .stroke(Theme.brandCyan.opacity(0.28), lineWidth: 1)
                        )
                )
            }
            .buttonStyle(.plain)
