...
struct TrainingProgramData {
    ...
    static let foundationsDays: [TrainingDay] = [
        ...
        TrainingDay(
            ...
            mainWork: [
                ...
                texLeveled("f_d2b_3", "Spanish Squat + Iso", 3, "30s", 45, .strength, ["Quads", "Knees"], "Lean back into band, hold deep position — progress through levels", cnsIgnition: true),
            ],
            isGated: false, isCompleted: false
        ),
        TrainingDay(
    ...
    static let flightDays: [TrainingDay] = [
        TrainingDay(
            ...
            mainWork: [
                ...
                tex("fl_d1a_5", "Tiered Plyometrics (regressed → progressed → deep)", 3, "6", 90, .plyometric, ["Full Body"], "Progress from low to med to high box — extensive to deep-tier contacts"),
    ...
    static let eliteDays: [TrainingDay] = [
        ...
        TrainingDay(
            ...
            mainWork: [
                ...
                tex("e_d1b_5", "Tiered Plyometrics (deep tier)", 3, "5", 120, .plyometric, ["Full Body"], "High box series with max intent — reactive, minimal ground contact"),
            ],
            isGated: false, isCompleted: false
        ),
        TrainingDay(
    ...
    private static func inferredPlyometricDepthTier(_ cat: Exercise.ExerciseCategory) -> PlyometricDepthTier {
        switch cat {
        case .mobility, .recovery: return .elasticityExtensive
        case .strength: return .intensityProgressed
        case .plyometric, .agility: return .reactiveDeep
        }
    }

    static func tex(
        _ id: String, _ name: String, _ sets: Int, _ reps: String, _ rest: Int,
        _ cat: Exercise.ExerciseCategory, _ muscles: [String], _ cues: String,
        tier: PlyometricDepthTier? = nil,
        cnsIgnition: Bool = false
    ) -> TrainingExercise {
        TrainingExercise(
            id: id, name: name, sets: sets, reps: reps, restSeconds: rest,
            category: cat, muscleGroups: muscles, cues: cues, progressionLevel: 1, hasLevels: false,
            plyometricDepthTier: tier ?? inferredPlyometricDepthTier(cat), cnsIgnition: cnsIgnition
        )
    }

    static func texLeveled(
        _ id: String, _ name: String, _ sets: Int, _ reps: String, _ rest: Int,
        _ cat: Exercise.ExerciseCategory, _ muscles: [String], _ cues: String,
        tier: PlyometricDepthTier? = nil,
        cnsIgnition: Bool = false
        ...
        TrainingExercise(
            id: id, name: name, sets: sets, reps: reps, restSeconds: rest,
            category: cat, muscleGroups: muscles, cues: cues, progressionLevel: 1, hasLevels: true,
            plyometricDepthTier: tier ?? inferredPlyometricDepthTier(cat), cnsIgnition: cnsIgnition
        )
    }
    ...
    static let pjfBandFoundationsDays: [TrainingDay] = [
        ...
        TrainingDay(
            ...
            mainWork: [
                texLeveled("pjf_f_d2_1", "Maximal Overcoming Hamstring Isometric", 3, "5-8s", 60, .strength, ["Hamstrings"], "Press into immovable resistance at max effort", cnsIgnition: true),
