...
struct BrainBrawlPlayView: View {
    ...
    private var duelOutcome: BrainBrawlDuelCanvasView.DuelOutcome? {
        guard let f = roundFeedback else { return nil }
    ...
    var body: some View {
        ZStack {
            BrainBrawlAcademyBackground(accentColor: accentColor)
            VStack(spacing: 24) {
                environmentHeader
                roundIndicator
                scoreRow
                BrainBrawlDuelCanvasView(accentColor: accentColor, outcome: duelOutcome)
        ...
        .onAppear {
            loadQuestions()
            if !FirstTimeBrainBrawlOverlay.hasSeen { showTutorialOverlay = true }
        }
        .overlay {
            if showTutorialOverlay {
                FirstTimeBrainBrawlOverlay(accentColor: accentColor) {
    ...
    private var scoreRow: some View {
        ...
        .padding(.vertical, 12)
        .accessibilityLabel("Score: You \(playerScore), AI \(opponentScore)")
    }
    ...
    private func questionCard(_ q: BrainBrawlQuestion) -> some View {
        ...
        .accessibilityLabel("\(q.subject): \(q.question)")
    }
