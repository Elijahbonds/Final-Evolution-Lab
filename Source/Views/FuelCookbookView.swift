...
// Fascial Hydration Broths, CNS Ignition Snacks, Dstrkt Protocol. DoorDash export placeholder.
...
struct FuelCookbookView: View {
    ...
    var body: some View {
        ...
        .alert("Export to cart", isPresented: $showExportAlert) {
            ...
            Text("DoorDash integration coming soon. Use \"Copy list\" to paste ingredients elsewhere.")
        }
    }
    ...
    private func exportTapped(_ recipe: CookbookRecipe) {
        if DoorDashExportService.exportToCart(ingredientNames: recipe.ingredients, mealPlanName: recipe.title) != nil {
            // Would open URL
        } else {
    ...
    private func copyList(_ recipe: CookbookRecipe) {
        let text = DoorDashExportService.groceryListText(ingredientNames: recipe.ingredients, title: recipe.title)
