...
/// Gold Master shard economy — outfits, blueprints, coaching, and **Exclusive Gear** (Unreal texture sync).
struct UFELShardMarketplaceView: View {
    ...
    @State private var showSovereignPurchaseFailed = false
    ...
    var body: some View {
        NavigationStack {
            ...
            .navigationTitle("Shard Marketplace")
            ...
            .alert("Sovereign API Unavailable", isPresented: $showSovereignPurchaseFailed) {
                Button("OK", role: .cancel) {}
            } message: {
                Text("Shard spending is blocked until the purchase service confirms your order. Your balance is unchanged. Try again when online.")
            }
            .onAppear {
                loadPurchased()
    ...
    private func completePurchase(_ item: ShopItem) {
        Task { @MainActor in
            let ok = await FELCreatorRevenueQueue.recordShardPurchase(
                item: item,
                shardCost: item.cost,
            ...
            guard ok else {
                ...
                showSovereignPurchaseFailed = true
                return
            }
            viewModel.profile.evolutionShards -= item.cost
            purchasedIds.insert(item.id)
            if let slot = item.gearSlot, let path = item.unrealGearTexturePath {
                viewModel.profile.equippedGearTexturePaths[slot] = path
            }
    ...
    var body: some View {
        Button(action: onTap) {
            VStack(alignment: .leading, spacing: 10) {
                ...
                if item.unrealGearTexturePath != nil {
                    Text("UNREAL SYNC")
                ...
                if let impact = item.performanceImpactLabel {
                    Text("PERFORMANCE: \(impact)")
                        .font(.system(size: 8, weight: .semibold, design: .monospaced))
                        .foregroundStyle(Theme.foundationGreen.opacity(0.95))
                        .lineLimit(3)
                        .fixedSize(horizontal: false, vertical: true)
                }

                if item.creatorMerchantID != nil {
                    Text("CREATOR PAYOUT · FELMerchantID")
...
typealias ShardShopView = UFELShardMarketplaceView
