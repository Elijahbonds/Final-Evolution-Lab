    ...
    private var exerciseHeader: some View {
        VStack(spacing: 16) {
            ZStack {
                ...
                // Video when available; otherwise 3D avatar performs the movement (no "VIDEO UNAVAILABLE" as primary).
                ZStack {
                    if demoEngine.isVideoAvailable, case .ready(let url) = demoEngine.videoLoadState {
                        ...
                        ZStack {
                            RealityKitExerciseAvatarView(exercise: exercise.toExercise())
                                .frame(height: 140)
                            ...
                            VStack {
                                ...
                                HStack(spacing: 8) {
                                    Text(exercise.category.rawValue.uppercased())
                                        ...
                                        .clipShape(Capsule())
                                    Text(exercise.plyometricDepthTier.shortLabel)
                                        .font(.system(size: 10, weight: .black, design: .monospaced))
                                        ...
                                        .clipShape(Capsule())
                                        .accessibilityLabel(exercise.plyometricDepthTier.movementDescription)
                                    if exercise.cnsIgnition {
                                        Text("CNS")
                                            .font(.system(size: 9, weight: .heavy, design: .monospaced))
                                            .foregroundStyle(Theme.elitePurple)
                                            .padding(.horizontal, 6)
                                            .padding(.vertical, 3)
                                            .background(Theme.elitePurple.opacity(0.15))
                                            .clipShape(Capsule())
                                    }
                                    if isCompleted {
                                        HStack(spacing: 4) {
                ...
                ZStack {
                    if !demoEngine.isVideoAvailable, case .loading = demoEngine.videoLoadState {
                        VStack(spacing: 12) {
                            ProgressView()
