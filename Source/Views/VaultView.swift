...
struct VaultView: View {
            ...
            HStack(spacing: 14) {
                ...
                VStack(alignment: .leading, spacing: 4) {
                    Text("Trade")
                        ...
                        .foregroundStyle(.white)
                    Text("Exchange shards or Creator Cards on the same Wi‑Fi (Academy Arena)")
    ...
    private var creatorCardsVaultSection: some View {
        let owned = CreatorCard.catalog.filter { viewModel.profile.ownsCard($0.id) }
        return VStack(alignment: .leading, spacing: 12) {
            HStack {
                Text("CREATOR CARDS")
                    ...
                    .tracking(2)
                Spacer()
                Text("\(owned.count) / \(CreatorCard.catalog.count)")
                    .font(.system(size: 11, weight: .bold, design: .monospaced))
                    .foregroundStyle(.tertiary)
            }
            if owned.isEmpty {
                Text("No cards in your vault yet. Unlock in the Lab or trade in person.")
                    ...
                    HStack(spacing: 12) {
                        ForEach(owned) { card in
                            ...
                            .background(
                                RoundedRectangle(cornerRadius: 14)
                                    .fill(Theme.cardBackground)
                                    .overlay(RoundedRectangle(cornerRadius: 14).strokeBorder(card.accentColor.opacity(0.35), lineWidth: 1))
                            )
                        }
                    }
                }
        ...
        .background(
            RoundedRectangle(cornerRadius: 18)
                ...
                .overlay(RoundedRectangle(cornerRadius: 18).strokeBorder(Theme.brandCyan.opacity(0.15), lineWidth: 1))
        )
    }
