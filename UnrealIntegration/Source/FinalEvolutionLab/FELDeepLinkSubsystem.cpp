...
const TMap<FString, FString>& GetVenueTokenToPackagePath()
{
	static TMap<FString, FString> T = [] {
		...
		M.Add(TEXT("Vault_Shop"), TEXT("/Game/FEL/Maps/Vault_Shop"));
		return M;
	}();
	return T;
}
	...
	const FString& OptionalArenaGameMode)
{
	...
	if (Primary.IsEmpty())
	{
		UE_LOG(LogTemp, Warning, TEXT("FEL FELBridge play: empty key."));
		return;
	}
	...
	if (PackagePath.IsEmpty())
	{
		UE_LOG(LogTemp, Warning, TEXT("FEL FELBridge play: could not resolve map for '%s' — add [FELPlayMap] in DefaultGame.ini."), *Primary);
		return;
	}
