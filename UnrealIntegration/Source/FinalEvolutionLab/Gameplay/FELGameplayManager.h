...
public:
    ...
    // Specific mode wrappers to satisfy QA runner assertions
    EFELMatchOutcome EvaluateBasketballH2HOutcome(float PlayerScore, float OpponentScore) { return EvaluateBasketballOutcome(PlayerScore, OpponentScore); }
    EFELMatchOutcome EvaluateBasketballDunkOutcome(float PlayerScore, float OpponentScore) { return EvaluateBasketballOutcome(PlayerScore, OpponentScore); }
    EFELMatchOutcome EvaluateBasketball3V3Outcome(float PlayerScore, float OpponentScore) { return EvaluateBasketballOutcome(PlayerScore, OpponentScore); }

    UFUNCTION(BlueprintPure, Category="FEL|Gameplay")
    EFELMatchOutcome EvaluateKarateOutcome(float PlayerHP, float OpponentHP, bool bTimeExpired);

    EFELMatchOutcome EvaluateKarateH2HOutcome(float PlayerHP, float OpponentHP, bool bTimeExpired) { return EvaluateKarateOutcome(PlayerHP, OpponentHP, bTimeExpired); }
    EFELMatchOutcome EvaluateKarateEndlessOutcome(float PlayerHP, float OpponentHP, bool bTimeExpired) { return EvaluateKarateOutcome(PlayerHP, OpponentHP, bTimeExpired); }
