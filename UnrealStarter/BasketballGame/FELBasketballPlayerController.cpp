...
void AFELBasketballPlayerController::PlayBondsBounceHaptics(EFELJumpTimingBand Band, float TimingLeakFactor)
{
	...
	if (Band == EFELJumpTimingBand::Perfect)
	{
		...
		PlayDynamicForceFeedback(1.f, 0.11f, false, false, true, false, ECollisionChannel::ECC_GameTraceChannel1, false);
		return;
	}
	...
	if (Band == EFELJumpTimingBand::Early || Band == EFELJumpTimingBand::Late || Leak < 0.75f)
	{
		...
		PlayDynamicForceFeedback(RumbleAmp, 0.42f, false, true, false, true, ECollisionChannel::ECC_GameTraceChannel1, false);
		return;
	}

	if (Band == EFELJumpTimingBand::Good)
	{
		PlayDynamicForceFeedback(0.62f, 0.09f, false, false, true, true, ECollisionChannel::ECC_GameTraceChannel1, false);
	}
}
...
void AFELBasketballPlayerController::BeginPlay()
{
	...
	const TSubclassOf<UFELNeuroDebugHUDWidget> WidgetClass =
		NeuroDebugHUDWidgetClass ? NeuroDebugHUDWidgetClass : UFELNeuroDebugHUDWidget::StaticClass();
...
void AFELBasketballPlayerController::OnIASprintLongPress(const FInputActionValue& Value)
{
	...
	{
		EnhancedInputSprint01 = FMath::Clamp(Value.GetMagnitudeF(), 0.f, 1.f);
	}
}
...
#endif
	const float Mag = FMath::Clamp(Value.GetMagnitudeF(), 0.f, 1.f);
	if (Mag < 0.22f)
	{
		return;
	}
