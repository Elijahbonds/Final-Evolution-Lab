...
void UFELNeuroMechanicBridgeSubsystem::HandlePostWorldInit(UWorld* World, const UWorld::InitializationValues& IVS)
{
	UNREFERENCED_PARAMETER(IVS);
	if (!World)
	{
		return;
	}
...
void UFELNeuroMechanicBridgeSubsystem::OnReadinessAppliedDebugPIE(const FFELReadinessSnapshot& S)
{
	...
	UE_LOG(LogTemp, Display, TEXT("[FEL Neuro] Readiness applied — effective JumpZ=%.1f cm/s, MaxWalkSpeed=%.1f uu/s | JSON: verticalEstimateInches=%.2f neuralDrive=%.2f prqScore=%.1f"),
		JumpZ, Walk, S.VerticalEstimateInches, S.NeuralDrive, S.PRQScore);

	if (GEngine)
	{
		const FString Msg = FString::Printf(
			TEXT("FEL Neuro: JumpZ=%.0f  Speed=%.0f  (vertEst=%.1f in, drive=%.0f)"),
			JumpZ, Walk, S.VerticalEstimateInches, S.NeuralDrive);
		GEngine->AddOnScreenDebugMessage(871u, 6.f, FColor::Cyan, Msg);
	}
#endif
}
