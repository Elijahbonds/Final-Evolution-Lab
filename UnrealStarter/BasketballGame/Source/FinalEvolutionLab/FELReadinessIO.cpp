...
bool FELReadinessIO::ParseSnapshotJsonString(const FString& JsonStr, FFELReadinessSnapshot& Out, FString* OutError, UWorld* WorldForTravel, bool* OutIssuedVenueTravel)
{
	...
	if (Root->TryGetStringField(TEXT("karateLabMode"), KarateLabStr))
	{
		...
		else if (L == TEXT("h2h") || L == TEXT("head_to_head") || L == TEXT("headtohead") || L == TEXT("storm") || L == TEXT("karate_h2h"))
		{
			Out.KarateLabVariant = EFELKarateLabMode::HeadToHeadStorm;
			Out.bKarateLabVariantFromHost = true;
		}
	}
	{
		...
		else if (Am == TEXT("karate_h2h") || Am == TEXT("karate_head_to_head") || Am == TEXT("karate_storm"))
		{
			Out.KarateLabVariant = EFELKarateLabMode::HeadToHeadStorm;
			Out.bKarateLabVariantFromHost = true;
		}
	...
	FString StoodCardDa;
	if (Root->TryGetStringField(TEXT("stoodCreatorCardDataAsset"), StoodCardDa) ||
		Root->TryGetStringField(TEXT("stood_creator_card_data_asset"), StoodCardDa))
	{
		Out.StoodCreatorCardDataAssetPath = StoodCardDa.TrimStartAndEnd();
	}
	...
	FString BBPrompt;
	if (Root->TryGetStringField(TEXT("brainBrawlInterestPrompt"), BBPrompt) ||
		Root->TryGetStringField(TEXT("brain_brawl_interest_prompt"), BBPrompt))
	{
		Out.BrainBrawlInterestPrompt = BBPrompt.TrimStartAndEnd();
	}
	FString BBOppPath;
	if (Root->TryGetStringField(TEXT("brainBrawlOpponentPathId"), BBOppPath) ||
		Root->TryGetStringField(TEXT("brain_brawl_opponent_path_id"), BBOppPath))
	{
		Out.BrainBrawlOpponentPathId = BBOppPath.TrimStartAndEnd().ToLower();
	}
...
bool FELReadinessIO::TryMandatoryVenueTravelForActiveMode(UWorld* World, const FFELReadinessSnapshot& Snap)
{
	...
	if (!FELArenaVenueTravel::ResolveOpenLevelName(Mode, TargetLevel))
	{
		UE_LOG(LogTemp, Verbose, TEXT("FELReadiness: mode not mapped to venue travel (active_mode='%s')."), *Snap.ActiveArenaMode);
		return false;
	}

	if (FELArenaVenueTravel::ShouldSkipTravelBecauseAlreadyOnVenue(Mode, Current))
	{
		UE_LOG(LogTemp, Log,
			TEXT("FELReadiness: already on venue '%s' (current='%s'); skip OpenLevel."),
			*FELArenaVenueTravel::GetVenueMatrixLabel(Mode),
			*Current);
		return false;
	}

	UE_LOG(LogTemp, Display,
		TEXT("FELReadiness: venue travel — matrix=%s active_mode='%s' current='%s' → OpenLevel %s"),
		*FELArenaVenueTravel::GetVenueMatrixLabel(Mode),
