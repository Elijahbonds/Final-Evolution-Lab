...
void UFELInputManager::PlayPushTwelveClinicalHaptic(const UObject* WorldContextObject, int32 BeatPhaseMod3)
{
	...
	if (Phase == 0)
	{
		PC->PlayDynamicForceFeedback(1.0f, 0.19f, true, false, true, false, EDynamicForceFeedbackAction::Start);
	}
	else
	{
		...
		PC->PlayDynamicForceFeedback(SnapIntensity, SnapDur, false, true, false, true, EDynamicForceFeedbackAction::Start);
	}
#endif
}
...
bool UFELInputManager::IsAnyGamepadConnected(const UObject* WorldContextObject)
{
	...
	for (FConstPlayerControllerIterator It = World->GetPlayerControllerIterator(); It; ++It)
	{
		...
		{
			// UE 5.7: UPlayerInput no longer exposes joystick enumeration; PlayerInput presence implies input stack is active.
			return true;
		}
	}
	return false;
