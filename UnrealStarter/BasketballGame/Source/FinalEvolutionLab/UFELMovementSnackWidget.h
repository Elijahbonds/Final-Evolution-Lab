...
protected:
	...
	/** When true, `TriggerSnackDemo` loads montage async before `PlaySnackDemonstration3D`. */
	UPROPERTY(EditAnywhere, Category = "FEL|MovementSnack")
	bool bUseAsyncDemonstrationLoad = false;

	UPROPERTY()
	TWeakObjectPtr<USkeletalMeshComponent> GhostDemonstrationMesh;

	void BeginDemonstrationPoseBlend(USkeletalMeshComponent* AvatarMesh, float BlendInSeconds);
	void BeginAsyncDemonstrationMontagePlay(USkeletalMeshComponent* GhostMeshOverride);
