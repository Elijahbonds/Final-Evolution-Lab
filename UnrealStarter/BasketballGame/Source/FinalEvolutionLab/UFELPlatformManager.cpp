...
void UFELPlatformManager::HandlePostWorldInit(UWorld* World, FWorldInitializationValues InitValues)
{
	(void)InitValues;
...
void UFELPlatformManager::InitializePlatformSettings()
{
	...
	{
		if (UFELGameInstance* FELGI = Cast<UFELGameInstance>(GI))
		{
			FELGI->ApplyProfileScalabilityToGameUserSettings();
		}
	}
}
...
void UFELPlatformManager::ApplyMacSettings()
{
	...
	// Apple Silicon (M-series): prefer hardware RT path when engine supports it on Metal (M3+ / project-dependent).
	{
		const FString Brand = FPlatformMisc::GetCPUBrand();
		const bool bAppleSilicon = Brand.Contains(TEXT("Apple")) || Brand.Contains(TEXT("M1")) || Brand.Contains(TEXT("M2"))
			|| Brand.Contains(TEXT("M3")) || Brand.Contains(TEXT("M4"));
		if (bAppleSilicon)
		{
			SetCVarInt32(TEXT("r.RayTracing"), 1);
