...
void UFELDemoManager::TriggerExerciseDemo()
{
	...
	const TSubclassOf<AFELExerciseDemonstrator> DClass =
		DemonstratorClass ? DemonstratorClass : AFELExerciseDemonstrator::StaticClass();
	...
	const float PRQ = static_cast<float>(GM->NeuroPRQScore);
	const float Leak = static_cast<float>(GM->NeuroKineticLeakageMultiplier);
...
void UFELDemoManager::ShowPerfectFormOverlay(const FFELArenaHUDConfig& Hud)
{
	...
	const TSubclassOf<UUserWidget> Cls =
		PerfectFormChecklistWidgetClass ? PerfectFormChecklistWidgetClass : UFELPerfectFormChecklistWidget::StaticClass();
