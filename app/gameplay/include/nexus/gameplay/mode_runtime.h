...
public:
  ...
  void reset();
  void updateFitnessSnapshot(const FitnessSnapshot& snapshot);
...
private:
  [[nodiscard]] auto currentPrqScore() const -> float;
  [[nodiscard]] auto currentNeuralDrive() const -> float;
  ...
  std::optional<FitnessSnapshot> m_fitnessSnapshot;
