// Spec §7.4 — PRQ readiness scoring for gameplay, HUD, and bridge telemetry.
#pragma once
...
class PRQEngine {
public:
  /// Baseline shown before any trusted sensor or scan metrics are available.
  [[nodiscard]] static auto getScore() -> float;
  [[nodiscard]] static auto getScore(const FitnessSnapshot& snapshot) -> float;

  /// Neural drive complements PRQ for arcade physics and burst affordances.
