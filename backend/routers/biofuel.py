...
async def scan_meal(req: ScanRequest, user: User = Depends(get_current_user)):
    ...
    if not EMERGENT_KEY or ImageContent is None or LlmChat is None or UserMessage is None:
        raise HTTPException(status_code=503, detail="Bio-Fuel vision scanning is unavailable until the AI provider key and SDK are configured")
