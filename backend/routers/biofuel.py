...
async def scan_meal(req: ScanRequest, user: User = Depends(get_current_user)):
    ...
    if not EMERGENT_KEY or not LlmChat or not UserMessage or not ImageContent:
        raise HTTPException(status_code=503, detail="BioFuel vision service is not configured")
