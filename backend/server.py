...
async def ai_coach(data: Dict[str, Any], user: User = Depends(get_current_user)):
    ...
    if not EMERGENT_KEY or LlmChat is None or UserMessage is None:
        return {"response":"AI coaching is not configured yet. Keep today focused: warm up, train one priority skill, log recovery, and review PRQ trends before the next session.","model":"fallback","type":prompt_type}
    try:
...
async def ai_chat(data: Dict[str, Any], user: User = Depends(get_current_user)):
    ...
    if not EMERGENT_KEY or LlmChat is None or UserMessage is None:
        return {"response":"AI chat is not configured yet. I can still help you navigate the lab: complete a system scan, pick a training mode, then log nutrition and recovery so your PRQ stays current.","model":"fallback","conversation_id":cid}
    try:
