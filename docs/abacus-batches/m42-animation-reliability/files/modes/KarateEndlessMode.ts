// KarateEndlessMode v3 — REPLACES the M35 file. Clip names routed through
// SPORT_CLIP: 'karate_idle_stance', 'karate_punch_light', 'karate_kick_roundhouse',
// 'karate_punch_heavy', and 'karate_block' were never real clips (the live
// rig's actual strike clips are jab/high_kick/uppercut/guard) — this is why
// fighters held a T-pose between hits. karate_hit_react and karate_knockdown
// WERE real (authored) and are unchanged. Also ships with the M42
// CameraDirector (fight preset pulled in 5.2→4.2, plus occlusion-fallback
// fix) — deploy that file alongside this one; it fixes the "camera pressed
// flat against a dojo wall" regression seen in the same audit.
  ...
  return {
    ...
    update(ctx, dt) {
      ...
      for (const mob of contacts) {
        ...
        if (!blocking) player.animator.play(SPORT_CLIP.karateHitReact, {});
