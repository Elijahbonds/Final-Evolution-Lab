// SkateRunMode v3 — REPLACES the M39 file. No gameplay logic changes; every
// hand-written clip name is routed through clipRegistry's SPORT_CLIP table
// (fixes the T-pose-while-riding defect) and the Rider now gets the v3
// GroundRide with its hard floor clamp (fixes the fall-through-the-world
// defect observed live). installSafePlay is wired in boardCore's buildRig,
// so this file only needs the clip-name swap.
  ...
  return {
    ...
    onInput(ctx: ModeContext, e: FelInput) {
      ...
      if (e.t === 'button' && e.pressed) {
        if (e.btn === 'A') {
          if (rig.rider.grounded) { rig.rider.jump(0.55 + pump * 0.45); rig.char.animator.play(SPORT_CLIP.boardAir, {}); }
...
// modeVerbs key `skateboard`: POP=A · KICKFLIP=B · HEELFLIP=Y · GRAB=X(hold) · PUMP=RT hold
