  ...
  return {
    ...
    onInput(ctx: ModeContext, e: FelInput) {
      ...
      if (e.t === 'button' && e.btn === 'A' && e.pressed && incoming && !swung) {
        ...
        if (q > 0.8) EffectsKit.burst(ctx.scene, ball.position.clone(), 'sparks');
    ...
    update(ctx: ModeContext, dt: number) {
      ...
      if (phase === 'flight') {
        ...
        if (!flying) {
          ...
          if (dist < 0.5) EffectsKit.burst(ctx.scene, holePos.add(new Vector3(0, 0.5, 0)), 'confetti');
  ...
  return {
    ...
    onInput(ctx: ModeContext, e: FelInput) {
      ...
      if (e.t === 'button' && e.btn === 'A' && e.pressed && incoming && !swung) {
        ...
        if (q > 0.85) EffectsKit.burst(ctx.scene, ball.position.clone(), 'confetti');
      ...
      if (phase === 'flight') {
        ...
        if (ball.position.z >= 10.9) {
          ...
          if (scored) {
            ...
            EffectsKit.burst(ctx.scene, new Vector3(0, 1.5, 11), 'confetti');
          } else {
