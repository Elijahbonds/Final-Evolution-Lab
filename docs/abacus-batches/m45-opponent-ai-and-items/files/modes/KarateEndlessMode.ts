...
export const KarateEndlessMode: ModeDefinition = (() => {
  ...
  async function spawnWave(ctx: ModeContext): Promise<void> {
    ...
    for (let i = 0; i < count; i++) {
      ...
      // opponent variety: a flanker mixed in means the ring doesn't read as
      // identical fighters closing in a straight line — one cuts an angle
  ...
  function ko(ctx: ModeContext, e: Enemy): void {
    ...
    EffectsKit.burst(ctx.scene, e.mob.char.root.position.add(new Vector3(0, 0.9, 0)), 'dust');
