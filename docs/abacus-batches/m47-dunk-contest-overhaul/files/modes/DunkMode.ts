// DunkMode v3 — "better than NBA Live 07's dunk contest." REPLACES the M44
// file. This is a structural rebuild, not a patch: Dunk Contest becomes an
// actual JUDGED SLAM DUNK CONTEST — 2 rounds, 2 dunks per round, 3 AI judges
// (reusing the Silk/Doc/Prime persona trio from the Cash Arena work) scoring
// difficulty, execution, and style live on every attempt, exactly like a
// real contest broadcast. Three things NBA Live's contest had that FEL's
// never did, now built:
//   1. PROPS — cycle with the d-pad before you charge: NONE, ALLEY-OOP (a
//      teammate tosses you the ball mid-air instead of you carrying it) or
//      OBSTACLE (clear a prop under the rim for a difficulty bonus, or clip
//      it and take a small penalty — never a hard fail).
//   2. A live judged scorecard with per-judge numbers and a personality
//      line, revealed dunk by dunk, contest total tracked across rounds.
//   3. A crowd HYPE meter that actually feeds the judges: a hot streak of
//      big scores measurably raises the next dunk's style score, the same
//      way a rocking arena visibly lifts real contest scores.
// Every existing reliability guarantee is kept: clipRegistry/installSafePlay
// (no bind pose), per-phase watchdogs (no stall, ever), camDirector.snapTo
// framing, SoundKit/EffectsKit, and gameFeel hit-stop/shake.
...
export const DunkMode: ModeDefinition = (() => {
  ...
  let playerTotal = 0, rivalTotal = 0, hype = 0;
  ...
  const def: ModeDefinition = {
    ...
    async load(ctx: ModeContext) {
      ...
      round = 1; dunkInRound = 0; playerTotal = 0; rivalTotal = 0; hype = 0; finishing = false;
      style = 'power'; prop = 'none';
      setPhase('approach');
      ctx.setHud({
        round: `${round}/${TOTAL_ROUNDS}`, dunkNum: `${dunkInRound + 1}/${DUNKS_PER_ROUND}`,
        score: playerTotal, rivalScore: rivalTotal, style: STYLE_LABEL[style], prop: PROP_LABEL[prop], hype: 0,
  ...
  async function finishAttempt(ctx: ModeContext, made: boolean): Promise<void> {
    ...
    if (!made) {
      ...
      lastScores = [];
      ctx.setHud({ banner: 'MISSED — 0 pts this attempt', judgeReveal: null });
    ...
    ctx.setHud({ score: playerTotal, hype: Math.round(hype), judgeReveal: scores });
    player.animator.play(SPORT_CLIP.dunkLandCrouch, { onEnd: () => player.animator.play(SPORT_CLIP.idle, { loop: true }) });
  ...
  function resetForNextAttempt(ctx: ModeContext): void {
    ...
    charge = 0; qteHit = false; qteWindowOpen = false; qteAccuracy = 0;
    void setupProp(ctx);
    ctx.camDirector.snapTo(player.root.position, rim);
    setPhase('approach');
    ctx.setHud({
...
// HUD CONTRACT ADDITIONS this file introduces:
//   dunkNum: string ('1/2')     — which dunk of the round
//   prop: string                — current prop selection label
//   hype: number (0-100)        — crowd meter
//   judgeReveal: JudgeScore[]|null — when non-null, HUD shows the 3-judge
//     scorecard (name/score/line) for ~2.6s before clearing itself
// All other fields (round, score, rivalScore, style, charge, slamPulse,
// hint, banner) use the same bare-value contract established in M42/M44 —
// no pre-labeled strings, the bezel decorates round/score itself.
