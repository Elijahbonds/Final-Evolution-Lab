// BasketballCore v2 — REPLACES the M48 file. Everything M48 shipped is kept
// (DribbleController, ShotMeter, DefenderBrain, TeammateBrain, contestLevel,
// clampToHalfCourt) and three systems are added for the 2K-Blacktop-style
// feel pass:
//   resolveBodyCollision — players can no longer ghost through each other.
//     Pairwise radial push-out on the XZ plane, mass-symmetric, cheap enough
//     to run on every pair every frame (15 pairs in 3v3).
//   Ankle-breaker — a crossover executed close to a tight defender now has a
//     consequence: checkAnkleBreak() tells the mode to stagger the defender
//     (brief stun the mode applies), which is the entire fantasy of the
//     Blacktop crossover game distilled to one readable beat.
//   Shot variety — classifyShot() reads the shooter's real context (distance,
//     movement direction relative to the hoop, contest) and types the attempt
//     as layup / floater / jumper / fadeaway, each with its own make%
//     modifier and meter tuning. Moving away from the hoop under contest is
//     now a harder-but-legal fadeaway instead of the same generic shot.
