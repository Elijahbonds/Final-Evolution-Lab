// ThreeVThreeMode v2 — REPLACES the M48 file. Wires in BasketballCore v2's
// Blacktop-feel systems across all six bodies:
//   BODY COLLISION — all 15 body pairs resolve every frame; screens and
//     bumping for position now physically happen instead of ghosting.
//   ANKLE-BREAKER — crossover in the nearest tight defender's face staggers
//     them 0.7s, opening the lane (same beat as 1v1).
//   SHOT VARIETY — my attempts are typed (layup/floater/jumper/fadeaway)
//     with per-type meter tuning and make%; HUD shows the type.
// Everything else from M48 is kept: TeammateBrain spacing/cuts, pass to the
// open man, assists, simulated opponent possessions where your D positioning
// sets their make% (same honest scope note — a fully manual 3-defender
// opponent offense is a separate, larger system).
...
export const ThreeVThreeMode: ModeDefinition = (() => {
  ...
  async function resolveMyShot(ctx: ModeContext, quality: ShotQuality): Promise<void> {
    ...
    if (made) {
      ...
      ctx.setHud({ score: myScore, banner: quality === 'perfect' ? `${label} — SPLASH!` : `${label} — GOOD!` });
    } else {
      SoundKit.play('miss');
...
// HUD fields: foeScore, target, ast, shotMeterT, time, and NEW shotType.
