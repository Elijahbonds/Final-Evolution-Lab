// modeVerbs v2 — REPLACES the M35 file. Fixes a live-confirmed wiring gap:
// the M48/M49 modes ('onevone', 'threevthree', 'carnival') never got entries
// here, so they fell back to `default` (a single generic ACTION=A button).
// That wasn't just cosmetic — 1v1/3v3 shooting is driven by a HELD TRIGGER
// (LocalInputSource reads `trigger R` for the shot meter), which the default
// overlay never emits. On touch, shooting was IMPOSSIBLE. Confirmed on the
// live build: 1v1 Hoops shows one "ACTION" chip and no way to shoot.
// Everything above the three new entries is byte-identical to M35.
