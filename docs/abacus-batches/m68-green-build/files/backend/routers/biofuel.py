...
# `emergentintegrations` is a private package that is not installable on a
# clean runner (it is pinned in requirements.txt but not on any public
# index). A hard import here made `server.py` unimportable, which took the
# ENTIRE backend — and every test that imports it — down with it. The LLM
# path is optional; the rest of this router is not. Degrade, don't collapse.
try:
    from emergentintegrations.llm.chat import ImageContent, LlmChat, UserMessage
    EMERGENT_LLM_AVAILABLE = True
except ImportError:  # pragma: no cover - depends on the deploy environment
    ImageContent = LlmChat = UserMessage = None  # type: ignore[assignment]
