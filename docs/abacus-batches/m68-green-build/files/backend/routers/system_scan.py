...
async def unified_system_scan(user: User = Depends(get_current_user)) -> UnifiedScanResponse:
    ...
    if game_count > 0:
        ...
        try:
            # game_sessions documents use BOTH key spellings in production:
            # routers/games.py writes "mode" for session docs (line ~271) and
            # "mode_id" for others (~186/209). Projecting only "mode_id" made
            # every "mode"-keyed session invisible, so modes_unlocked came back
            # empty for real users. Read both.
            mode_docs = await db.game_sessions.find(
                {"user_id": user.user_id}, {"_id": 0, "mode_id": 1, "mode": 1}
            ).to_list(500)
            modes_unlocked = list({
                m for d in mode_docs
                for m in (d.get("mode_id"), d.get("mode"))
                if m
            })
        except Exception:
            modes_unlocked = []
