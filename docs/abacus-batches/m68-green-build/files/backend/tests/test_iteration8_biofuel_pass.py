...
if not BASE_URL:
    # Fall back to the repo's frontend/.env, resolved RELATIVE TO THIS FILE.
    # This used to be a hardcoded "/app/frontend/.env" opened at import time,
    # so on any machine without that exact path the open() raised during
    # COLLECTION — which aborts the whole pytest run and meant all 225 tests
    # never executed. Missing config must skip this module, not kill the suite.
...
if not BASE_URL:
    pytest.skip(
        "REACT_APP_BACKEND_URL not set and frontend/.env not found — "
        "this module needs a running backend. Set the env var to run it.",
