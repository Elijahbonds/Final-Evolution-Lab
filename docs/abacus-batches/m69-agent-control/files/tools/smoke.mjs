...
async function checkMode(context, mode) {
  ...
  try {
    ...
    const startBtn = page.locator('text=/tap to start|^\\s*start\\s*$|press start/i').first();
    if (await startBtn.count().catch(() => 0)) {
      await startBtn.click({ force: true, timeout: 5000 }).catch(() => {});
    } else {
    ...
    if (!reachedPlaying) {
      ...
      problems.push(`never reached 'playing' — stuck at "${st}". The start gate was not cleared, `
        + 'so nothing below this point has actually been exercised.');
    }
