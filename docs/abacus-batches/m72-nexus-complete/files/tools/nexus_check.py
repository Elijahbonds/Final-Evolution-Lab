...
def check_nexus_registry() -> None:
    """Registry ↔ descriptors ↔ loader.

    These were WARNs while nothing loaded the descriptors — a missing file
    broke nothing, so failing the gate would have been noise. NexusSceneLoader
    now exists and every mode loads from JSON, so the same conditions are real
    runtime defects and are reported as ERRORs. The gate got stricter because
    the code got more capable, not because the rule changed.
    """
    ...
    scene_dir = os.path.join(ROOT, reg.get("sceneDirectory", "FinalEvolutionLab/Resources/NexusScenes"))
    venues = reg.get("venues", [])

    # Is there a loader at all? Everything below is only an ERROR if so.
    swift_src = ""
    for f in product_swift_files():
        with open(f, encoding="utf-8", errors="replace") as fh:
            swift_src += fh.read()
    has_loader = "NexusSceneLoader" in swift_src
    report = err if has_loader else warn

    if not has_loader:
        warn("nexus",
             "no NexusSceneLoader in the Swift sources, yet NexusProject.json describes scenes as "
             "JSON-defined. Descriptors are orphaned data — wire the loader or stop describing the "
             "architecture as data-driven.",
             "NexusStarter/NexusProject.json")

    missing = [(v.get("modeId"), v["scene"]) for v in venues
               if v.get("scene") and not os.path.exists(os.path.join(scene_dir, v["scene"]))]
    if missing:
        report("nexus",
               f"{len(missing)} of {len(venues)} venues reference descriptors that do not exist in "
               f"{reg.get('sceneDirectory')}: {', '.join(m for m, _ in missing)}. "
               "Regenerate with: bash tools/nexus_scenes.sh --write",
               "VenueRegistry.nexus.json")

    # STRUCTURAL VALIDATION — catches the exact bug that shipped undetected.
    # The original descriptor was valid JSON and completely undecodable:
    # Swift synthesizes Codable for an enum-with-associated-values as
    # {"skeleton":{...}} and for CGPoint as [x, y]. Hand-written JSON used
    # {"type":"skeleton",...} and {"x":..,"y":..}. "It parses" proves nothing;
    # these two shape rules are what "it loads" actually depends on.
    CASES = {"physics", "surface", "sprite", "skeleton", "trigger", "camera", "light"}
    if os.path.isdir(scene_dir):
        for v in venues:
            s = v.get("scene")
            if not s:
                continue
            path = os.path.join(scene_dir, s)
            ...
            try:
                ...
                report("nexus", f"{s} is not valid JSON: {e}", s)
                continue

            if doc.get("gameModeId") != v.get("modeId"):
                report("nexus",
                       f"{s} declares gameModeId '{doc.get('gameModeId')}' but the registry files it "
                       f"under '{v.get('modeId')}' — the loader rejects this mismatch at runtime.", s)

            for ent in doc.get("entities", []):
                pos = ent.get("transform", {}).get("position")
                if not isinstance(pos, list):
                    report("nexus",
                           f"{s}: entity '{ent.get('id')}' has transform.position as "
                           f"{type(pos).__name__}, but CGPoint decodes from a [x, y] ARRAY. "
                           "This is the exact shape that made the original descriptor undecodable.", s)
                    break
                for comp in ent.get("components", []):
                    if not isinstance(comp, dict) or len(comp) != 1 or next(iter(comp)) not in CASES:
                        report("nexus",
                               f"{s}: entity '{ent.get('id')}' has a component shaped "
                               f"{list(comp)[:3] if isinstance(comp, dict) else comp} — Swift expects a "
                               "single-key object keyed by the case name, e.g. "
                               '{"skeleton": {"category": "Plyometric", "amplitude": 1}}.', s)
                        break
    ...
    if os.path.exists(proj_p):
        ...
        ids = {v.get("modeId") for v in venues}
        if start and start not in ids:
            err("nexus", f"startScene '{start}' is not a modeId in the venue registry", "NexusProject.json")


def check_toolchain_floor(files: list[str]) -> None:
    """Language features that pin a MINIMUM Swift/Xcode version.

    Found the hard way: a real Swift 6.0.3 compiler rejected 147 declarations
    that are perfectly correct Swift 6.1. Nothing in the repo said 6.1 was
    required — AGENTS.md claimed 'Xcode 15+', which cannot even open an
    objectVersion 77 project. A floor nobody wrote down is a floor somebody
    discovers on a build machine.
    """
    # `nonisolated` applied to a TYPE (column 0) is SE-0449, Swift 6.1+.
    # Applied to a MEMBER (indented) it has been legal since 5.5 — so the
    # column matters, and matching it loosely would produce nonsense.
    pat = re.compile(r"^nonisolated\s+(?:final\s+)?(struct|enum|class|actor|extension)\b")
    hits: list[str] = []
    for f in files:
        with open(f, encoding="utf-8", errors="replace") as fh:
            n = sum(1 for line in fh if pat.match(line))
        if n:
            hits.append(f"{rel(f)} ({n})")
    if hits:
        total = sum(int(h.rsplit("(", 1)[1][:-1]) for h in hits)
        warn("toolchain",
             f"{total} `nonisolated` type declarations across {len(hits)} files require "
             "Swift 6.1+ / Xcode 16.3+ (SE-0449). Swift 6.0 rejects every one. This is a "
             "REQUIREMENT, not a defect — do not 'fix' the declarations.",
    ...
    if pbx:
        with open(pbx, encoding="utf-8", errors="replace") as fh:
            proj = fh.read()
        m = re.search(r"objectVersion = (\d+);", proj)
        if m and int(m.group(1)) >= 77:
            warn("toolchain",
                 f"project.pbxproj is objectVersion {m.group(1)} and uses synchronized root "
                 "groups — Xcode 16+ only. Xcode 15 cannot open this project.",
                 rel(pbx))
