...
export const VENUE_SPECS: Record<string, NexusWebSpec> = {
  ...
  // ── creative ────────────────────────────────────────────────────────────
  // The Nexus mode registry never included the creative disciplines, so none
  // of them had a venue. Dance needs one most: it is the only creative mode
  // where the AVATAR is the content, so it needs a stage, a key light on the
  // performer, and a crowd to perform to.
  dance: {
