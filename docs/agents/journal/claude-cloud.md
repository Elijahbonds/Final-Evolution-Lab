...
## 2026-07-28T09:00Z · claude-cloud
DID: M101 Pass 2 Phase 8 — answered "can a match be verified for money" by measurement instead of authoring eight SimulatableMode implementations. tools/determinism_probe.mjs + core/verifiability.ts, 22 tests.
TOUCHED: docs/abacus-batches/m101-pass2-verifiability/**, tools/determinism_probe.mjs, tools/certify.mjs
FOUND: DUNK IS CLEAN, and it corrects me. Over a full 40-second contest, every one of 24,558 Math.random() calls resolved to Babylon's particle system — zero from game logic. M94's README says M63 rolled the rival with Math.random() and that this is the single call making dunk contests unverifiable. THE DEPLOYED BUILD DOES NOT DO THAT. I carried the assumption from reading M63's source and never checked the build.
FOUND: four modes DO roll a gameplay decision. onevone and threevthree call `e4.decide < er.poll < es.poll` (107 and 85 times) — an AI opponent deciding from an input poll. tennis rolls inside a key-up handler, so a shot's outcome depends on an unreproducible number at release. karate-vs calls `tm.decide < Object.update`. One named call site each; the fix is the same shape every time.
FOUND: MY OWN THRESHOLD WAS WRONG AND WOULD HAVE PASSED THE FAILING MODE. onevone makes 107 AI calls against 13,794 particle calls — a 0.9% share — and my first version used a 20% threshold. One unreproducible decision invalidates the whole match; share is the wrong metric entirely. There is now a test asserting the 0.9% case is disqualifying.
FOUND: three ways the probe was wrong before it was right. (1) Counting calls: the first run said 8/8 UNVERIFIABLE and the top site was startPositionFunction — particles — a false alarm on every mode in the product. (2) Classifying on one stack frame: the immediate caller is a shared minified helper (`s` is Babylon RandomRange); the frame above identifies the subsystem. (3) Sampling: 1-in-97 cannot find an AI roll happening four times a match, which is exactly what is being looked for. It now captures every call up to 60k, and a sampled mode can only ever be reported inconclusive, never clean.
...
## 2026-08-23T00:00Z · claude-cloud
DID: M104 — the mixamorig10: prefix fix, confirmed against FEL's real production asset, and a dependency-free offline GLB re-skin tool (glb_reskin.mjs). 26 tests pass, fully offline against a synthetic GLB.
TOUCHED: docs/abacus-batches/m104-mixamo-numbered-prefix-and-reskin-tool/**, m80-external-animation/files/anim/boneNames.ts, m80-external-animation/files/tools/clip_check.mjs, m80-external-animation/files/tests/anim_test.ts, m65-avatar-system-phase0/files/tools/fel_conform.py, docs/DUNK-FIRST.md, docs/abacus-batches/DRAG-AND-DROP.md
FOUND: A REAL, PREVIOUSLY UNDISCOVERED LANDMINE, CONFIRMED AND FIXED. Downloaded FEL's own production base mesh (male_athlete_base_model_fbx) straight from its live source URL and read its real bone names with assimp — every one of its 65 joints is prefixed `mixamorig10:`, not `mixamorig:` or `mixamorig1:`. All three copies of the strip-prefix rule (boneNames.ts, clip_check.mjs, fel_conform.py) enumerated a fixed list and missed it. fel_conform.py's probe() is the worst case: it reported conforms:True — zero prefixed bones found — on the exact rig that is not conformed. Fixed all three with one regex, /^mixamorig\d*[:_](.+)$/i, instead of a list that is always one export behind.
FOUND: I DID NOT CLAIM TO FIX THE T-POSE, AND HERE IS WHY. Blender is unavailable in this environment, so I built a dependency-free GLB reader/writer in plain Node and a proximity-based re-skinning algorithm (nearest bone segment, inverse-distance weighted, the plain version of Blender's Automatic Weights) rather than stop at "someone needs Blender." Measuring the real downloaded asset directly: Ch28_Body is 59.4% Head-dominant, but its vertices cluster almost entirely in the face/neck/hands region of a fully-clothed character — that share is likely CORRECT, not the M98 bug.
FOUND: I CHECKED THE PART THAT WOULD ACTUALLY SHOW THE BUG AND IT WAS ALREADY RIGHT. Ch28_Hoody has real sleeve geometry to the wrist. Restricting to sleeve vertices past the shoulder: 734 of 734 — 100% — already bound to an arm-family bone, before any repair. My own working hypothesis (the jacket is the culprit) was contradicted by directly measuring it, and I reported the contradiction instead of forcing the narrative.
FOUND: THE RAW ASSET AND THE DEPLOYED ASSET ARE NOT THE SAME FILE. This raw download's four meshes sum to 20,431 vertices. M98 measured the live deployed character (Body_c5) at 18,409. Some transformation happens between "raw Mixamo download" and "what the game actually renders" that no batch code in this repo performs and that I could not observe from here — no cached session credentials in this fresh container to re-authenticate and capture the real deployed .glb URL from the network tab.
FOUND: caught by running my own tests, twice. First draft asserted exact single-winner outcomes for vertices placed exactly AT joint positions — legitimate floating-point ties (distance-0 from two adjacent segments simultaneously), not algorithm bugs. Repositioned test vertices off-axis to be geometrically representative, and changed one aggregate assertion from `< 0.5` to `<= 0.5` with a comment explaining the deterministic tie, rather than force a false pass. Verified determinism by running the suite three times.
NEEDS: claude-mini or whoever has Blender — do NOT assume the raw downloaded asset is what's broken and start re-exporting it blind. The sleeve — the one region directly checkable — was already correct in the raw source. Extract the ACTUAL deployed .glb (network tab on the live app) and run `node tools/glb_reskin.mjs inspect <file>` on that first; it gives the same measurement M98 took live, offline, in about a second.
NEEDS: abacus — ship the mixamorig10: prefix fix on its own if M80's boneNames.ts is already integrated. It is a one-file diff and every day it isn't shipped is a day FEL's own base character would silently fail fel_conform.py's own conformance check.
NEXT: get the actual deployed character asset into a session that can run glb_reskin.mjs against it — that observation, not another hypothesis, is what closes the T-pose item on docs/DUNK-FIRST.md.
