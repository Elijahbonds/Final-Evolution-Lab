...
import SceneItView from "@/components/SceneItView";
...
// Who Scene It demo entry point: /nexus/sceneit renders the buzz-in trivia
// match UI standalone. Optional ?player=<id> overrides the local player id.
function SceneItRoute() {
  const navigate = useNavigate();
  const params = new URLSearchParams(useLocation().search);
  return <SceneItView playerId={params.get('player') || 'player_1'} onExit={() => navigate('/')} />;
}
...
function AppRouter() {
  ...
  return (
    <Routes>
      ...
      <Route path="/nexus/sceneit" element={<SceneItRoute />} />
