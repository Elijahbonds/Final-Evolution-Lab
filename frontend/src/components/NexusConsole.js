...
const LOG_POOL = [
  ...
  "NEXUS bridge channel open — runtime acknowledged",
  ...
  const fetchStatus = useCallback(async () => {
    try {
      ...
      setSubsystems({
        ...
        unreal:    { status: d.unreal?.status || "ready", detail: d.unreal?.detail || "NEXUS runtime bridge loaded" },
      });
  ...
  const fetchStatus = useCallback(async () => {
    try {
      ...
      setSubsystems({
        ...
        healthkit: { status: "authorized", detail: "steps · HRV · sleep" },
        websocket: { status: "connected",  detail: "ws://emergent · 38ms" },
        unreal:    { status: "loading",    detail: "awaiting NEXUS runtime signal" },
      });
    }
  }, []);
