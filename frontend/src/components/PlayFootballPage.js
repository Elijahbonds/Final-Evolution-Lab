...
import { FootballMode } from '@/game/modes/football/index.js';
import FootballHUD from '@/game/modes/football/FootballHUD.js';
import GamepadOverlay from '@/game/input/GamepadOverlay.js';
import { ModePhase } from '@/game/modes/GameModeInterface.js';

const POLL_MS = 100;

/** Playable football slice. Route: /play/football */
export default function PlayFootballPage() {
  ...
  const startMatch = useCallback(async () => {
    ...
    try {
      ...
      console.error('[PlayFootballPage] failed to start', err);
      setStatus('error');
    }
  }, []);

  useEffect(() => {
    const mode = new FootballMode('football', canvasRef.current, containerRef.current);
  ...
  return (
    <div
      ref={containerRef}
      style={{
        position: 'fixed', inset: 0, background: '#0c1210', overflow: 'hidden',
    ...
    >
      ...
      <div style={{ position: 'absolute', top: 14, left: 14, zIndex: 60 }}>
        <FootballHUD score={s?.score ?? 0} prq={s?.prq ?? 50} yardsGained={s?.yardsGained ?? 0} down={s?.down ?? 1} toGo={s?.toGo ?? 10} touchdowns={s?.touchdowns ?? 0} lastMove={s?.lastMove ?? ''} speed={s?.speed ?? 1} isBursting={s?.isBursting ?? false} winner={s?.winner ?? null} />
      </div>
        ...
        <div style={overlayStyle}>
          <div style={{ display: 'grid', gap: 14, justifyItems: 'center' }}>
            <div style={{ color: s?.winner === 'player' ? '#facc15' : '#f87171', fontSize: 34, fontWeight: 900 }}>
              {s?.winner === 'player' ? 'DRIVE COMPLETE' : 'DRIVE OVER'}
            </div>
