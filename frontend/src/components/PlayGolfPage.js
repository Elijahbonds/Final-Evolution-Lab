...
import { GolfMode } from '@/game/modes/golf/index.js';
import GolfHUD from '@/game/modes/golf/GolfHUD.js';
import GamepadOverlay from '@/game/input/GamepadOverlay.js';
import { ModePhase } from '@/game/modes/GameModeInterface.js';

const POLL_MS = 100;

/** Playable Golf links slice. Route: /play/golf */
export default function PlayGolfPage() {
  ...
  const startMatch = useCallback(async () => {
    ...
    try {
      ...
      console.error('[PlayGolfPage] failed to start', err);
      setStatus('error');
    }
  }, []);

  useEffect(() => {
    const mode = new GolfMode('golf', canvasRef.current, containerRef.current);
  ...
  return (
    <div
      ref={containerRef}
      style={{
        position: 'fixed', inset: 0, background: '#0c140e', overflow: 'hidden',
    ...
    >
      ...
      <div style={{ position: 'absolute', top: 14, left: 14, zIndex: 60 }}>
        <GolfHUD score={s?.score ?? 0} prq={s?.prq ?? 50} hole={s?.hole ?? 1} totalHoles={s?.totalHoles ?? 9} par={s?.par ?? 3} strokes={s?.strokes ?? 0} distanceToPin={s?.distanceToPin ?? 150} lastShotDistance={s?.lastShotDistance ?? 0} lastResult={s?.lastResult ?? ''} swingCharging={s?.swingCharging ?? false} swingPower={s?.swingPower ?? 0} winner={s?.winner ?? null} />
      </div>
        ...
        <div style={overlayStyle}>
          <div style={{ display: 'grid', gap: 14, justifyItems: 'center' }}>
            <div style={{ color: s?.winner === 'player' ? '#facc15' : '#f87171', fontSize: 34, fontWeight: 900 }}>
              {s?.winner === 'player' ? 'ROUND COMPLETE' : 'ROUND COMPLETE'}
            </div>
