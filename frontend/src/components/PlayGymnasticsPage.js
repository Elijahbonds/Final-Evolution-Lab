...
import GymnasticsMode from '@/game/modes/gymnastics2/GymnasticsMode.js';
import GamepadOverlay from '@/game/input/GamepadOverlay.js';

const POLL_MS = 100;

/** Gymnastics vault. Route: /play/gymnastics — alternate □/○ to sprint, △ flips, ✕ stick */
export default function PlayGymnasticsPage() {
  ...
  const startRound = useCallback(async () => {
    ...
    try {
      ...
      console.error('[PlayGymnasticsPage] failed to start', err);
      setStatus('error');
    }
  }, []);

  useEffect(() => {
    const mode = new GymnasticsMode('gymnastics', canvasRef.current, containerRef.current);
  ...
  return (
    <div ref={containerRef} style={{ position: 'fixed', inset: 0, background: '#0b1018', overflow: 'hidden', fontFamily: 'system-ui, sans-serif' }}>
      ...
      <div style={{ position: 'absolute', top: 14, left: 14, zIndex: 60, background: 'rgba(8,10,16,0.85)', border: '1px solid rgba(255,255,255,0.1)', borderRadius: 14, padding: 14, color: '#f8fafc', minWidth: 220 }}>
        <div style={{ fontSize: 11, letterSpacing: '0.1em', color: '#94a3b8' }}>VAULT</div>
      ...
      {status === 'loading' && <div style={overlayStyle}><div style={{ color: '#e2e8f0', fontSize: 18, fontWeight: 700 }}>CHALKING UP…</div></div>}
