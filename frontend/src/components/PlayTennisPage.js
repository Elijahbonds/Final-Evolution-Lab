...
import { TennisMode } from '@/game/modes/tennis/index.js';
import TennisHUD from '@/game/modes/tennis/TennisHUD.js';
import GamepadOverlay from '@/game/input/GamepadOverlay.js';
import { ModePhase } from '@/game/modes/GameModeInterface.js';

const POLL_MS = 100;

/** Playable Tennis rally slice. Route: /play/tennis */
export default function PlayTennisPage() {
  ...
  const startMatch = useCallback(async () => {
    ...
    try {
      ...
      console.error('[PlayTennisPage] failed to start', err);
      setStatus('error');
    }
  }, []);

  useEffect(() => {
    const mode = new TennisMode('tennis', canvasRef.current, containerRef.current);
  ...
  return (
    <div
      ref={containerRef}
      style={{
        position: 'fixed', inset: 0, background: '#10160c', overflow: 'hidden',
    ...
    >
      ...
      <div style={{ position: 'absolute', top: 14, left: 14, zIndex: 60 }}>
        <TennisHUD sets={s?.sets ?? [0, 0]} games={s?.games ?? [0, 0]} points={s?.points ?? ['0', '0']} score={s?.score ?? 0} prq={s?.prq ?? 50} lastResult={s?.lastResult ?? ''} powerCharge={s?.powerCharge ?? 0} serveReady={s?.serveReady ?? true} inRally={s?.inRally ?? false} winner={s?.winner ?? null} />
      </div>
