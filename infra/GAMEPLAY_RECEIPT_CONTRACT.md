# Gameplay receipt authority (iOS / UE / Runtime)
...
| **`sessionBound`** | UE/Runtime JSON over an authenticated channel **without** a cryptographic server receipt. History allowed; **no ranked PRQ / ranked economy** in Release. | Default when bridge omits `fel_trust_level` |
...
- **UE / Runtime:** `ue:<session_id>` from `session_id`, `ue_session_id`, `emergent_session_id` (historical), `game_session_id`, or `fel_session_id`. Optional explicit `fel_receipt_id` wins.
...
## UE / Runtime (`applyVerifiedPayload` → `ingestVerifiedGameplayReceipt`)
