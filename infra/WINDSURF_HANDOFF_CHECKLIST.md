...
## 4. Release desktop reference bundle
...
This copies **`release-reference/*.md`**, **`release-reference/desktop-bundle/*`**, and **`SHIPPING_ARCHITECTURE.md`** into **`~/Desktop/FEL-Release-Reference`** (override with **`DEST=`**).

Point Windsurf’s workspace or context at **`FEL-Release-Reference`** so architecture lock and pipe docs stay attached to local builds.
...
- [ ] **`sync_release_desktop.sh`** run so **`SHIPPING_ARCHITECTURE.md`** is on the Desktop reference bundle.
