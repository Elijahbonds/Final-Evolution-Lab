...
struct AvatarDemoView: View {
    ...
    var body: some View {
        ...
        .onAppear {
            withAnimation(.easeInOut(duration: exerciseCycleDuration).repeatForever(autoreverses: true)) {
                phase = 1
            }
    ...
    private var skeletonFigure: some View {
        Canvas { context, size in
            ...
            for bone in bones {
                ...
                path.move(to: from)
                path.addLine(to: to)
                context.stroke(path, with: .color(limbColor.opacity(0.8)), style: StrokeStyle(lineWidth: 3, lineCap: .round))
            }

            let glowRadius: CGFloat = glowPulse ? 8 : 4
