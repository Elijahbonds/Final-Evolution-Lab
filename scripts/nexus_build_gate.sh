...
reset_stale_cmake_compiler_cache() {
  local build_dir="$1"
  local cache="${build_dir}/CMakeCache.txt"
  if [[ -f "${cache}" && -n "${CXX:-}" ]] && ! grep -q "^CMAKE_CXX_COMPILER:.*=${CXX}$" "${cache}"; then
    cmake -E rm -rf "${cache}" "${build_dir}/CMakeFiles"
  fi
}

echo "==> Phase 1: headless build (NEXUS_ENABLE_RENDERER=OFF)"
reset_stale_cmake_compiler_cache build-headless
...
reset_stale_cmake_compiler_cache build-full
