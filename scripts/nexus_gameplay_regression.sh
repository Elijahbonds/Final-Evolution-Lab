...
if [[ "${SKIP_BUILD}" -eq 0 ]]; then
  ...
  if [[ -f "${HEADLESS_DIR}/CMakeCache.txt" && -n "${CXX:-}" ]]; then
    cached_cxx="$(grep -E '^CMAKE_CXX_COMPILER:FILEPATH=' "${HEADLESS_DIR}/CMakeCache.txt" 2>/dev/null | cut -d= -f2- || true)"
    if [[ -n "${cached_cxx}" && "${cached_cxx}" != "$(command -v "${CXX}" 2>/dev/null || printf '%s' "${CXX}")" ]]; then
      echo "==> Resetting stale headless CMake compiler cache (${cached_cxx} -> ${CXX})"
