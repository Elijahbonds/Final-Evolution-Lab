...
production_modes = [
    ...
    "basketball_3v3", "karate_h2h", "baseball", "football", "soccer", "golf",
...
modes_exercised = [
    m
    for m in production_modes
    if f"PASS: sprint mode={m}" in gameplay_log
    or f"mode_id={m}" in gameplay_log
    or f"mode={m}" in gameplay_log
]
...
payload = {
    ...
    "production_live_modes_expected": len(production_modes),
    "production_live_modes_seen_in_log": len(modes_exercised),
    "production_live_modes": modes_exercised,
