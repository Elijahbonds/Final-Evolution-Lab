...
if [[ "${SKIP_BUILD}" -eq 0 ]]; then
  ...
  SELECTED_CXX_PATH="$(command -v "${CXX:-}" 2>/dev/null || true)"
  if [[ -n "${SELECTED_CXX_PATH}" && -f "${HEADLESS_DIR}/CMakeCache.txt" ]] &&
     ! grep -Fq "CMAKE_CXX_COMPILER:FILEPATH=${SELECTED_CXX_PATH}" "${HEADLESS_DIR}/CMakeCache.txt"; then
    echo "==> Refreshing stale CMake compiler cache for CXX=${CXX}"
    rm -rf "${HEADLESS_DIR}/CMakeCache.txt" "${HEADLESS_DIR}/CMakeFiles"
  fi
