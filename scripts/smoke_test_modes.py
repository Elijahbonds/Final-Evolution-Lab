...
def test_swift_enum():
    ...
    all_modes = PRODUCTION_MODES + STAGING_MODES + PREVIEW_MODES
    for mode in all_modes:
        # Search for rawValue; split Swift modes may expose a NEXUS runtime alias instead.
        alias_markers = SWIFT_RUNTIME_ALIASES.get(mode, [])
        if f'= "{mode}"' in content or all(marker in content for marker in alias_markers):
