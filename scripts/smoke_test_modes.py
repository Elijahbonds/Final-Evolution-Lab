...
Tests each runtime production mode's registration, configuration, and deep link routing.
Run against the checked-in registries; no live backend is required.
"""
import json
import sys
import os
...
PLAYABLE_CONFIG_MODES = PRODUCTION_MODES + STAGING_MODES

MODE_ALIASES = {
    # iOS and legacy venue registries split the NEXUS C++ dunk runtime into a 3D
    # scene alias and an IRL camera alias. Either satisfies the runtime id.
    "basketball_dunk": ["basketball_dunk_3d"],
}


def mode_candidates(mode):
...
def test_ue_mode_maps():
    ...
    all_modes = PLAYABLE_CONFIG_MODES
...
def test_venue_registry():
    ...
    all_modes = PLAYABLE_CONFIG_MODES
    for mode in all_modes:
        entry_id = next((candidate for candidate in mode_candidates(mode) if candidate in mode_ids), None)
        if entry_id is not None:
            entry = next(m for m in vr["modes"] if m["id"] == entry_id)
...
def test_swift_enum():
    ...
    for mode in all_modes:
        # Search for rawValue
        if any(f'= "{candidate}"' in content for candidate in mode_candidates(mode)):
...
def main():
    ...
    print("  18 runtime production modes · registry aliases/non-game/preview checked")
    print("═══════════════════════════════════════════════════════════")
