...
SWIFT_MODE_ALIASES = {
    # iOS exposes user-facing IRL/3D dunk choices; NEXUS runtime keeps one C++ mode id.
    ...
    for mode in all_modes:
        ...
        if registry_mode in mode_ids:
            ...
            if entry["venueKey"] in venue_keys:
                alias_note = f" via {registry_mode}" if registry_mode != mode else ""
...
def test_swift_enum():
    ...
    for mode in all_modes:
        ...
        if f'= "{mode}"' in content:
            ...
            aliases = ", ".join(SWIFT_MODE_ALIASES[mode])
            ok(f'{mode} maps to Swift enum alias: {aliases}')
        else:
