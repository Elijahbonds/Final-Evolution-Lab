...
# ═══════════════════════════════════════════════════════════════════════════════
# Production runtime modes expected to pass all NEXUS gameplay gates.
# ═══════════════════════════════════════════════════════════════════════════════
PRODUCTION_MODES = [
...
APP_ONLY_PRODUCTION_MODES = ["basketball_dunk_3d", "basketball_dunk_irl"]
BACKEND_PRODUCTION_MODES = PRODUCTION_MODES + APP_ONLY_PRODUCTION_MODES
NON_GAME_MODULES = ["market_browse"]
PREVIEW_MODES = ["movement_lab"]

UE_EXEMPT_MODES = {"basketball_dunk_irl", "movement_lab"}
SWIFT_RAW_VALUE_ALIASES = {"basketball_dunk": "basketball_dunk_3d"}
UE_DESCRIPTOR_MODES = PRODUCTION_MODES + ["basketball_dunk_3d"] + NON_GAME_MODULES
VENUE_REGISTRY_MODES = (
    [mode for mode in PRODUCTION_MODES if mode != "basketball_dunk"]
    + APP_ONLY_PRODUCTION_MODES
    + NON_GAME_MODULES
)
...
def test_mode_manager_registry():
    ...
    if declared_total == len(registry):
        ...
        fail(f"ModeManager total_modes={declared_total}, actual={len(registry)}")
    ...
    for mode in BACKEND_PRODUCTION_MODES:
        if mode in registry:
            info = registry[mode]
...
def test_ue_mode_maps():
    ...
    all_modes = BACKEND_PRODUCTION_MODES + NON_GAME_MODULES
    for mode in all_modes:
        if mode in mode_map:
            if mode_map[mode] is None and mode in UE_EXEMPT_MODES:
                ok(f"{mode} → no UE map (expected)")
            elif mode_map[mode] is None:
                fail(f"{mode} has null UE map without exemption")
            else:
...
def test_arena_settings():
    ...
    for mode in UE_DESCRIPTOR_MODES:
        if mode in modes:
            cfg = modes[mode]
...
def test_swift_enum():
    ...
    swift_modes = [mode for mode in PRODUCTION_MODES if mode != "basketball_dunk"]
    swift_modes += APP_ONLY_PRODUCTION_MODES + NON_GAME_MODULES
    for mode in swift_modes:
        raw_value = SWIFT_RAW_VALUE_ALIASES.get(mode, mode)
        # Search for rawValue
        if f'= "{raw_value}"' in content:
    ...
    if 'case "basketball_dunk":' in content and ".basketballDunkContest3D" in content:
        ok("basketball_dunk C++ runtime alias resolves to Swift 3D dunk")
    else:
        fail("basketball_dunk C++ runtime alias missing from Swift playableMode")
...
def test_economy_integration():
    ...
    scoring_modes = BACKEND_PRODUCTION_MODES  # app-facing production receipts need weights
