#!/usr/bin/env python3
"""
FEL Smoke Test Suite — NEXUS production runtime acceptance tests.
Tests each production runtime mode's registration, configuration, and deep link
routing. Swift may expose split UX ids for a canonical C++ runtime id; aliases
below keep the smoke aligned with that source-of-truth boundary.
"""
import json
import sys
import os
...
def test_venue_registry():
    ...
    all_modes = PRODUCTION_MODES + STAGING_MODES + PREVIEW_MODES
    for mode in all_modes:
        entry = next((m for m in vr["modes"] if m["id"] == mode or m.get("nexusRuntimeModeId") == mode), None)
...
def test_swift_enum():
    ...
    all_modes = PRODUCTION_MODES + STAGING_MODES + PREVIEW_MODES
    for mode in all_modes:
        swift_mode = SWIFT_MODE_ALIASES.get(mode, mode)
        # Search for rawValue
...
def main():
    ...
    print(f"  {len(PRODUCTION_MODES)} runtime modes · 8 test categories · Registry → Economy")
    print("═══════════════════════════════════════════════════════════")
