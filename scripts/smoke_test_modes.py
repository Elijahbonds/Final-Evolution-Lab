...
Tests each mode's registration, configuration, and deep link routing.
...
def gameplay_modes():
    return PRODUCTION_MODES + STAGING_MODES + PREVIEW_MODES

def public_app_modes():
    return [
        mode for mode in gameplay_modes() + NON_GAME_MODULES
        if mode not in LEGACY_RUNTIME_ALIASES
    ]
...
def test_mode_manager_registry():
    ...
    for mode in REGISTRY_ONLY_PREVIEW_MODES:
        if mode in registry and registry[mode]["status"] == "preview":
            ok(f"{mode} → registry-only preview (expected)")
        elif mode in registry:
...
def test_ue_mode_maps():
    ...
    all_modes = gameplay_modes() + NON_GAME_MODULES
...
def test_arena_settings():
    ...
    all_modes = [
        mode for mode in gameplay_modes() + NON_GAME_MODULES
        if ue_maps.get(mode) is not None
    ]
    for mode in all_modes:
        if mode in modes:
            cfg = modes[mode]
...
def test_economy_integration():
    ...
    # Verify PRQ weights cover all scoring gameplay modes, including staging/preview loops.
    scoring_modes = gameplay_modes()
