...
SWIFT_SHIPPING_PRODUCTION_MODES = [mode for mode in PRODUCTION_MODES if mode != "basketball_dunk"]
IRL_ONLY_MODES = {"basketball_dunk_irl"}
UE_BACKED_MODES = [mode for mode in PRODUCTION_MODES if mode not in IRL_ONLY_MODES]
...
def test_ue_mode_maps():
    ...
    all_modes = UE_BACKED_MODES + STAGING_MODES
...
def test_swift_enum():
    ...
    all_modes = SWIFT_SHIPPING_PRODUCTION_MODES + STAGING_MODES
    for mode in all_modes:
        # Search for rawValue
