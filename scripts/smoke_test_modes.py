...
SPLIT_DUNK_SHIPPING_MODES = ["basketball_dunk_3d", "basketball_dunk_irl"]
...
def test_mode_manager_registry():
    ...
    for mode in PRODUCTION_MODES + SPLIT_DUNK_SHIPPING_MODES:
        if mode in registry:
            info = registry[mode]
...
def test_ue_mode_maps():
    ...
    all_modes = PRODUCTION_MODES + [m for m in SPLIT_DUNK_SHIPPING_MODES if m not in IRL_MODES] + NON_GAME_MODULES
...
def test_venue_registry():
    ...
    all_modes = PRODUCTION_MODES + SPLIT_DUNK_SHIPPING_MODES + NON_GAME_MODULES
    for mode in all_modes:
        if mode in mode_ids:
...
def test_economy_integration():
    ...
    scoring_modes = PRODUCTION_MODES + SPLIT_DUNK_SHIPPING_MODES
