...
def test_arena_settings():
    ...
    all_modes = PRODUCTION_MODES + STAGING_MODES
    for mode in all_modes:
        if mode in ue_maps and ue_maps[mode] is None:
            ok(f"{mode} → IRL/null UE map (ArenaSettings not required)")
            continue
        if mode in modes:
            cfg = modes[mode]
...
def test_fel_play_map():
    ...
    for mode in all_modes:
        if mode in ue_maps and ue_maps[mode] is None:
            ok(f"{mode} → IRL/null UE map (FELPlayMap not required)")
            continue
        if mode in play_map:
            path = play_map[mode]
...
def test_swift_enum():
    ...
    for mode in all_modes:
        # Search for rawValue
        if f'= "{mode}"' in content or f'case "{mode}":' in content:
