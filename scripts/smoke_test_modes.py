#!/usr/bin/env python3
"""
FEL Smoke Test Suite — registry, routing, app enum, and economy acceptance tests.
Tests production, staging, preview, IRL, and non-game module mode alignment.
Run against repository fixtures or a live/mock FEL backend.
"""
import json
import sys
import os
...
PREVIEW_NON_GAME_MODULES = ["movement_lab"]
...
def test_mode_manager_registry():
    ...
    for mode in PREVIEW_NON_GAME_MODULES:
        if mode in registry and registry[mode]["status"] == "preview":
            ok(f"{mode} → preview non-game module (expected)")
        elif mode in registry:
...
def test_arena_settings():
    ...
    for mode in all_modes:
        if mode in IRL_MODES:
            ok(f"{mode} → IRL mode (ArenaSettings not required)")
            continue
        if mode in modes:
            cfg = modes[mode]
...
def test_venue_registry():
    ...
    for mode in all_modes:
        ...
        elif mode in RUNTIME_ALIAS_MODES and "basketball_dunk_3d" in mode_ids:
            ok(f"{mode} → runtime alias covered by basketball_dunk_3d VenueRegistry")
        else:
...
def test_fel_play_map():
    ...
    for mode in all_modes:
        if mode in IRL_MODES:
            ok(f"{mode} → IRL mode (FELPlayMap not required)")
            continue
        if mode in play_map:
            path = play_map[mode]
...
def test_swift_enum():
    ...
    all_modes = PRODUCTION_MODES + STAGING_MODES + PREVIEW_MODES + NON_GAME_MODULES
    for mode in all_modes:
        if mode in RUNTIME_ALIAS_MODES:
            ok(f'{mode} runtime alias does not require a Swift enum case')
            continue
        # Search for rawValue
...
def main():
    ...
    print("  22 registry modes · 8 test categories · Registry → Economy")
    print("═══════════════════════════════════════════════════════════")
