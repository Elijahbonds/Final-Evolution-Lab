    ...
    if ue_maps_path.exists():
        ...
        for mode_id, unreal_map in ue_maps.items():
            if unreal_map is None:
                continue
            alias = LEGACY_MAP_ALIASES.get(mode_id)
            if mode_id not in play_map_section and (not alias or alias not in play_map_section):
...
def validate_arena_settings():
    ...
    if mgr_path.exists():
        ...
        registry = mgr.get("mode_manager", {}).get("mode_registry", {})
        for mode_id, info in registry.items():
            if mode_id in IRL_MODE_IDS or info.get("render_mode") == "IRL":
                continue
            settings_id = mode_id if mode_id in modes else LEGACY_MAP_ALIASES.get(mode_id, mode_id)
            if settings_id not in modes:
