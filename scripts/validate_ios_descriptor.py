...
def is_irl_or_null_map(mode_id, info=None, ue_maps=None):
    """IRL modes are camera/Vision flows and intentionally do not cook UE maps."""
    if info and info.get("render_mode") == "IRL":
        return True
    if ue_maps is not None and mode_id in ue_maps and ue_maps[mode_id] is None:
        return True
    return False
...
def validate_arena_settings():
    ...
    if mgr_path.exists():
        ...
        registry = mgr.get("mode_manager", {}).get("mode_registry", {})
        for mode_id, info in registry.items():
            if is_irl_or_null_map(mode_id, info, ue_maps):
                continue
