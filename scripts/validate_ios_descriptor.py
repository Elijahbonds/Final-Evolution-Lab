...
def validate_fel_play_map():
    ...
    if ue_maps_path.exists():
        ...
        for mode_id, unreal_map in ue_maps.items():
            if mode_id in IRL_MODES or unreal_map is None:
                continue
            play_map_id = LEGACY_UE_MODE_ALIASES.get(mode_id, mode_id)
...
def validate_arena_settings():
    ...
    if mgr_path.exists():
        ...
        registry = mgr.get("mode_manager", {}).get("mode_registry", {})
        for mode_id, info in registry.items():
            if mode_id in IRL_MODES:
                continue
            arena_mode_id = LEGACY_UE_MODE_ALIASES.get(mode_id, mode_id)
