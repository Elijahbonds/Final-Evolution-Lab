...
def is_irl_mode(mode_id, info=None, ue_map=_MISSING_MAP):
    """IRL modes are camera-native and intentionally do not have UE map packages."""
    if info and info.get("render_mode") == "IRL":
        return True
    if ue_map is None:
        return True
    return mode_id.endswith("_irl")
...
def validate_fel_play_map():
    ...
    if ue_maps_path.exists():
        ...
        for mode_id, map_token in ue_maps.items():
            if is_irl_mode(mode_id, ue_map=map_token):
                continue
...
def validate_arena_settings():
    ...
    if mgr_path.exists():
        ...
        if ue_maps_path.exists():
            ue_maps = json.loads(ue_maps_path.read_text()).get("mode_to_unreal_map", {})
        for mode_id, info in registry.items():
            map_token = ue_maps[mode_id] if mode_id in ue_maps else _MISSING_MAP
            if is_irl_mode(mode_id, info, map_token):
                continue
