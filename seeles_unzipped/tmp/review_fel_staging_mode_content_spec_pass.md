...
- **Key detail**:
  ...
  - cooked_runtime_note: "[FELPlayMap] in DefaultGame.ini is the cooked iOS runtime map path source of truth"
...
FEL_ModeManager.production.json  (mode status gate — EXISTS)
  -> [FELPlayMap] in DefaultGame.ini  (cooked iOS runtime path — ABSENT)
...
4. **`infra/ue5_config/DefaultGame.ini`**
   [FELPlayMap] section entries for court_carnival and who_scene_it.
...
4. **DefaultGame.ini [FELPlayMap] format is unknown.** No .ini template, schema, or example exists in workspace. Modifying agent must derive format from UE5 DefaultGame.ini conventions (or request a schema from the architecture blueprint).
