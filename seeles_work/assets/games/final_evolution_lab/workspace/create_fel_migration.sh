...
{
  "cooked_runtime_note": "[EmergentPlayMap] in DefaultGame.ini is the cooked iOS runtime map path source of truth. /Game/FEL/Maps paths in this JSON are logical map/venue tokens used by the backend.",
  "mode_manager": {
    "total_modes": 19,
    "production_modes": 14,
    "mode_registry": {
...
# ── FILE 7: FELEmergentDeepLinkSubsystem.h ───────────────────────────────────
cat > UnrealIntegration/Source/FinalEvolutionLab/FELEmergentDeepLinkSubsystem.h << 'HEREDOC'
#pragma once
...
UCLASS()
class FINALEVOLUTIONLAB_API UFELEmergentDeepLinkSubsystem : public UGameInstanceSubsystem
{
    GENERATED_BODY()
public:
...
HEREDOC
echo "✓ FILE 7: FELEmergentDeepLinkSubsystem.h"
...
void UFELEmergentDeepLinkSubsystem::Initialize(FSubsystemCollectionBase& Collection) { Super::Initialize(Collection); }

const TMap<FString, FString>& UFELEmergentDeepLinkSubsystem::GetModeToVenueMap() {
...
void UFELEmergentDeepLinkSubsystem::HandleDeepLink(const FString& URL) {
    FString Prefix = TEXT("fel://play/");
    if (URL.StartsWith(Prefix)) LaunchMode(URL.RightChop(Prefix.Len()));
}

void UFELEmergentDeepLinkSubsystem::LaunchMode(const FString& ModeId) {
