...
- `infra/ue5_config/DefaultGame.ini` — [EmergentPlayMap] entries (this is distinct from `Config/DefaultGame.ini`)
...
- `UnrealIntegration/Source/FinalEvolutionLab/FELEmergentDeepLinkSubsystem.h`
...
- `infra/ue5_config/DefaultGame.ini` (the real [EmergentPlayMap] version)
...
6. **The "Emergent" name is a legacy term** — `UFELEmergentDeepLinkSubsystem` is the UE5 deep-link routing subsystem (handles `fel://play/` URLs → map paths). It is NOT a mock AI agent. The user confirmed (Round 21 log) they "replaced legacy emergent mock integrations with standard Google GenAI SDK calls in backend/core.py" — but `core.py` is on the macOS machine, not in this workspace.
...
- **Notable conventions**:
  - `[EmergentPlayMap]` in `infra/ue5_config/DefaultGame.ini` = cooked iOS runtime map path table. The term "Emergent" here means the deep-link routing subsystem, NOT a third-party service.
...
- **Role**: [EmergentPlayMap] entries for all 19 modes — cooked iOS runtime map path source of truth
...
- **Note**: The `Config/DefaultGame.ini` IN workspace is a DIFFERENT file — it only has `[/Script/EngineSettings.GeneralProjectSettings]` entries (project name, version, company). The [EmergentPlayMap] file lives at `infra/ue5_config/DefaultGame.ini` on the macOS machine.
...
- **Key entries**: `who_scene_it → Neuro_Arena`, `court_carnival → Venice_Beach_Court`, `market_browse → Sovereign_Shop`
...
- **Entrypoint for all files NOT in workspace**: server.py, ue_mode_maps.json, ArenaSettings.json, GameMode.swift, MentalResiliencyEngine.swift, FELEmergentDeepLinkSubsystem.h/.cpp, FELNeuroCognitiveSubsystem.h/.cpp, infra/ue5_config/DefaultGame.ini, infra/ios/ExportOptions.plist, infra/fel_prebuild_ci_check.sh, and the 2 environment layout JSON files
...
iOS URL scheme: fel://play/{mode_id}
  → UFELEmergentDeepLinkSubsystem::HandleDeepLink()
...
- Venue routing is handled by `infra/ue5_config/DefaultGame.ini` [EmergentPlayMap] entries and `backend/ue_mode_maps.json` (both defined only in migration script heredocs)
...
- `infra/ue5_config/DefaultGame.ini` (macOS only, in migration script) = [EmergentPlayMap] routing
- The [EmergentPlayMap] section referenced in the ModeManager JSON note lives in the SECOND file
...
- "emergent" in `[EmergentPlayMap]` = mode-to-venue routing table name, coined by this project
