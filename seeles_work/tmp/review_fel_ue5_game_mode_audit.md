...
├── Config/
│   ├── DefaultGame.ini     ← [EmergentPlayMap] 19 venue route entries
...
Player selects mode (WBP_ModeSelect → OnModeTileSelected)
  ...
  → [EmergentPlayMap] looked up from DefaultGame.ini → UE map load
...
- **DefaultGame.ini EmergentPlayMap** — all 19 entries are complete and correct. Do not add or rename without updating both the macOS project and the registry.
