...
[macOS]/infra/ue5_config/DefaultGame.ini  (contains [EmergentPlayMap] for all 19 modes)
[macOS]/FinalEvolutionLab/Models/GameMode.swift
[macOS]/FinalEvolutionLab/Services/MentalResiliencyEngine.swift
[macOS]/UnrealIntegration/Source/FinalEvolutionLab/FELEmergentDeepLinkSubsystem.h
...
- DefaultGame.ini [EmergentPlayMap] section (workspace DefaultGame.ini has only [GeneralProjectSettings])
...
8. **Config/DefaultGame.ini in workspace** contains only GeneralProjectSettings (Phase 8 appended); the [EmergentPlayMap] block is in the macOS-only infra/ue5_config/DefaultGame.ini.
...
- **Relevant configs**:
  ...
  - [EmergentPlayMap] entries: macOS only in infra/ue5_config/DefaultGame.ini (all 19 modes mapped)
- **Architecture**:
...
### Config files
- `Config/DefaultGame.ini` — ONLY has [/Script/EngineSettings.GeneralProjectSettings]: ProjectID, ProjectName, ProjectVersion=1.0.0, CompanyName. NO [EmergentPlayMap] section.
...
- [EmergentPlayMap] lives at [macOS]/infra/ue5_config/DefaultGame.ini (all 19 modes mapped to /Game/FEL/Venues/ paths)
...
| Mode | C++ Class | Outcome Evaluator | ModeWeight | Arena Settings | Environment Layout | [EmergentPlayMap] Entry |
|---|---|---|---|---|---|---|
...
**Bottom line**: 14 modes fully implemented in C++ (classes + evaluators). 5 modes stub-only (registry + ArenaSettings + EmergentPlayMap only).
...
### To fix DefaultGame.ini for [EmergentPlayMap]:
- The workspace Config/DefaultGame.ini has NO [EmergentPlayMap] entries — these are in the macOS-only `infra/ue5_config/DefaultGame.ini`.
- If the workspace DefaultGame.ini needs to be extended, add the [EmergentPlayMap] block from the heredoc in create_fel_migration.sh.
