...
type MedicalDisclaimerGatewayProps = {
  open: boolean;
  /** Production PWA shell — opened after user acknowledges disclaimer. */
  labUrl: string;
  onClose: () => void;
};

/**
 * Clinical gate: must be acknowledged before entering the Lab PWA.
 * z-index 1100 — above hero, HUD, and pricing chrome.
 */
export function MedicalDisclaimerGateway({ open, labUrl, onClose }: MedicalDisclaimerGatewayProps) {
  useEffect(() => {
    if (!open) return;
