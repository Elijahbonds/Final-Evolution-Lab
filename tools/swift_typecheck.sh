...
# list because tools/swift-shims/ provides Linux stand-ins for them — that is
# what lifts coverage from 38 files to 44.
collect() {
  python3 - <<'PY'
import glob, re
APPLE_ONLY = {"SwiftUI","UIKit","SceneKit","SpriteKit","CoreGraphics","CoreMotion",
...
mkdir -p "$WORK/src"
for f in "${FILES[@]}"; do
  sed -e 's/^import OSLog$//' -e 's/^import QuartzCore$//' "$f" > "$WORK/src/$(echo "$f" | tr / _)"
done
