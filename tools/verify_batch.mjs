  ...
  while (i < n) {
    ...
    switch (state) {
      case 'code':
        ...
        else if (closers.has(c)) { if (stack.pop() !== c) return false; }
        if (!/\s/.test(c)) prev = c;
        i++;
        continue;
      case 'line':
      ...
      case 'tpl': {
        ...
        // a template literal's ${...} is real code, but its braces balance
        // within the literal either way, so tracking depth adds no accuracy
        i++;
        continue;
      }
    }
...
async function verifyBatch(batchDir) {
  ...
  if (!existsSync(filesDir)) {
    // doc-only batches are legitimate (specs, wiring notes)
    warn(name, 'no files/ directory — treating as a documentation-only batch');
    return;
  }
